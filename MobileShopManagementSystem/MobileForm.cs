...
namespace MobileShopManagementSystem
{
    public partial class MobileForm : Form
    {
        ...
        public void DisplayMobileData()
        {
            ...
            string query = "SELECT * FROM mobile_info";
        ...
        private void AddButton_Click(object sender, EventArgs e)
        {
            ...
            {   // Создаем запрос к базе данных для проверки существования записи с таким же значением ID
                string checkIDQuery = "SELECT COUNT(*) FROM mobile_info WHERE ID = @ID";
                ...
                string query = "INSERT INTO mobile_info (ID, Brand, Model, Price, Quantity) VALUES " +
        ...
        private void UpdateButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string countQuery = "SELECT COUNT(*) FROM mobile_info";
                ...
                string price = PriceField.Text;
                string quantity = QuantityField.Text;
                string RAM = RAMField.Text;
                string ROM = ROMField.Text;
                string color = ColorField.Text;
                string SIM = SIMField.Text;
                string camera = CameraField.Text;
                string battery = BattaryField.Text;
                string date = DateField.Text;

                // Проверка на пустые поля
                if (string.IsNullOrEmpty(ID) || string.IsNullOrEmpty(brand) || string.IsNullOrEmpty(model) ||
                    string.IsNullOrEmpty(price) || string.IsNullOrEmpty(quantity) || string.IsNullOrEmpty(RAM) ||
                    string.IsNullOrEmpty(ROM) || string.IsNullOrEmpty(color) || string.IsNullOrEmpty(SIM) ||
                    string.IsNullOrEmpty(camera) || string.IsNullOrEmpty(battery) || string.IsNullOrEmpty(date))
                {
                    MessageBox.Show("Пожалуйста, заполните все поля");
                    return;
                }
                ...
                // Проверка поля Color на наличие только английских символов и не более 10 символов
                if (!Regex.IsMatch(color, @"^[a-zA-Z]+$") || color.Length > 10)
                {
                    MessageBox.Show("Поле 'Color' должно содержать только английские символы и не более 10 символов");
                    return;
                }

                // Проверка полей Price, Quantity, RAM, ROM, SIM, Camera, Battery на наличие только цифр
                if (!Regex.IsMatch(price, @"^\d+$") || !Regex.IsMatch(quantity, @"^\d+$") || !Regex.IsMatch(RAM, @"^\d+$") ||
                    !Regex.IsMatch(ROM, @"^\d+$") || !Regex.IsMatch(SIM, @"^\d+$") || !Regex.IsMatch(camera, @"^\d+$") ||
                    !Regex.IsMatch(battery, @"^\d+$"))
                {
                    MessageBox.Show("Поля 'Price', 'Quantity', 'RAM', 'ROM', 'SIM', 'Camera', 'Battery' должны содержать только цифры");
                    return;
                }

                // Запрос к базе данных для обновления записи
                string updateQuery = $"UPDATE mobile_info SET Price = '{price}', Quantity = '{quantity}' WHERE ID = '{ID}' AND Brand = '{brand}' AND Model = '{model}' AND Color = '{color}' AND RAM = '{RAM}' AND ROM = '{ROM}' AND SIM = '{SIM}' AND Camera = '{camera}' AND Battery = '{battery}'AND Date = '{date}' ";
        ...
        private void DeleteButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string deleteQuery = $"DELETE FROM mobile_info WHERE ID = '{ID}' AND Brand = '{brand}' AND Model = '{model}' AND Price = '{price}' AND Quantity = '{quantity}' AND RAM = '{RAM}' AND ROM = '{ROM}' AND Color = '{color}' AND SIM = '{SIM}' AND Camera = '{camera}' AND Battery = '{battery}' AND Date = '{date}'";
        ...
        private void ClearButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string deleteQuery = "DELETE FROM mobile_info";
        ...
        private void TableToPDF_Click(object sender, EventArgs e)
        {
            ...
            string outputPath = $@"C:\Users\30100\source\repos\MobileShopManagementSystem\MobileShopManagementSystem\Mobile\PDF\output_{currentTime}_{currentDate}.pdf";
            ...
            try
            {
                // Создание команды для выборки данных из таблицы mobile_info
                string query = "SELECT * FROM mobile_info";
                MySqlCommand command = new MySqlCommand(query, database.GetConnection());
        ...
        private void TableToWord_Click(object sender, EventArgs e)
        {
            ...
            string outputPath = $@"C:\Users\30100\source\repos\MobileShopManagementSystem\MobileShopManagementSystem\Mobile\Word\output_{currentTime}_{currentDate}.docx";
            ...
            try
            {
                ...
                Word.Table wordTable = document.Tables.Add(document.Range(), dataTable.Rows.Count + 1, dataTable.Columns.Count);
        ...
        private void TableToExcel_Click(object sender, EventArgs e)
        {
            ...
            string outputPath = $@"C:\Users\30100\source\repos\MobileShopManagementSystem\MobileShopManagementSystem\Mobile\Excel\output_{currentTime}_{currentDate}.xlsx";
